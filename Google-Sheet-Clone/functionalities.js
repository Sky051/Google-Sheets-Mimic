...
let do_avg = document.querySelector("#my_avg");

do_avg.addEventListener('click', () => {
    var selectedValues = [];

    selectedCells.forEach(cell => {
        let value = parseFloat(cell.innerText.trim()); // Convert to number
        if (!isNaN(value)) {
    ...
    if (selectedValues.length > 0) {
        let avg = selectedValues.reduce((a, b) => a + b, 0) / selectedValues.length;
        let sum = selectedValues.reduce((a, b) => a + b, 0);
        let count = selectedValues.length;
        let min = Math.min(...selectedValues);
        let max = Math.max(...selectedValues);
        // Update the average display in the frontend
        document.querySelector("#avg-text").innerText = `Avg: ${avg.toFixed(2)}`;
        document.querySelector("#sum-text").innerText = `Sum: ${sum.toFixed(2)}`;
        document.querySelector("#ct-text").innerText = `Count: ${count.toFixed(2)}`;
        document.querySelector("#min-text").innerText = `Min: ${min.toFixed(2)}`;
        document.querySelector("#max-text").innerText = `Max: ${max.toFixed(2)}`;
        // console.log(`Average: ${avg.toFixed(2)}`);
    } else {
        // No numerical values selected
        document.querySelector("#sum-text").innerText = "Avg: 0";
        console.log("No numerical values selected!");
    }
